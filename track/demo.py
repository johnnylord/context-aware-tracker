...
class DemoTrack(BaseTrack):
    ...
    def register(self, feature):
        ...
        if len(self.feature_pool) >= DemoTrack.POOL_SIZE:
            self.feature_pool = self.feature_pool[1:]

    def update_head(self, keypoints):
        self.keypoints = keypoints
        # 0, 15, 16, 17, 18
        # ys = keypoints[[0, 15, 16, 17, 18], :]
        ys = keypoints[[0, 15, 16, 17, 18], 1]
        ys = ys[ys > 0]
        y = np.mean(ys)
        self.ylist.append(y)
        if len(self.ylist) > 2:
